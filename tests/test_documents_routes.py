...
from studio_app.routes.documents import purge_documents, reindex_documents, upload_document
...
async def test_purge_documents_deletes_all_chunks_for_tenant(admin_pool: Pool) -> None:
    ...
    try:
        async with _simulate_request_connection():
            await upload_document(file=_md_upload_file("leave.md", _VALID_MD), section_role="hr", tenant_id=None)
        async with _simulate_request_connection():
            await upload_document(
                file=_md_upload_file("budget.md", b"## Ngan sach\nChi tieu quy 1.\n"),
                section_role="finance",
                tenant_id=None,
            )
        async with _simulate_request_connection():
            result = await purge_documents(tenant_id=None)
    finally:
        middleware._request_session.reset(token)  # type: ignore[arg-type]
...
async def test_purge_documents_does_not_touch_other_tenant(admin_pool: Pool) -> None:
    tenant_a = await _seed_tenant(admin_pool, "documents-probe-i-a")
    tenant_b = await _seed_tenant(admin_pool, "documents-probe-i-b")
    admin_a_id = await _seed_user(admin_pool, tenant_a, "admin-a@acme.com", ["admin"])
    admin_b_id = await _seed_user(admin_pool, tenant_b, "admin-b@acme.com", ["admin"])
    await _seed_section(admin_pool, tenant_a, "hr", admin_a_id)
    await _seed_section(admin_pool, tenant_b, "hr", admin_b_id)

    token_a = _set_session(tenant_id=tenant_a, user="admin-a@acme.com", roles=["admin"])
    try:
        async with _simulate_request_connection():
            await upload_document(file=_md_upload_file("leave.md", _VALID_MD), section_role="hr", tenant_id=None)
    finally:
        middleware._request_session.reset(token_a)  # type: ignore[arg-type]

    token_b = _set_session(tenant_id=tenant_b, user="admin-b@acme.com", roles=["admin"])
    try:
        async with _simulate_request_connection():
            await upload_document(file=_md_upload_file("leave.md", _VALID_MD), section_role="hr", tenant_id=None)
        async with _simulate_request_connection():
            await purge_documents(tenant_id=None)
    finally:
        middleware._request_session.reset(token_b)  # type: ignore[arg-type]

    async with admin_pool.connection() as conn:
        await conn.execute("SELECT set_config('app.tenant_id', %s, true)", (str(tenant_a),))
        cur = await conn.execute("SELECT count(*) FROM kb.chunks WHERE tenant_id = %s", (str(tenant_a),))
        row = await cur.fetchone()
    assert row is not None
    assert row[0] == 1, "purge của tenant B không được đụng vào chunk của tenant A"


async def test_company_admin_cannot_purge_other_tenant(admin_pool: Pool) -> None:
    tenant_a = await _seed_tenant(admin_pool, "documents-probe-j-a")
    tenant_b = await _seed_tenant(admin_pool, "documents-probe-j-b")
    ...
    try:
        with pytest.raises(HTTPException) as exc_info:
            async with _simulate_request_connection():
                await purge_documents(tenant_id=str(tenant_b))
        assert exc_info.value.status_code == 403
    finally:
        middleware._request_session.reset(token)  # type: ignore[arg-type]


async def test_reindex_documents_recomputes_embeddings(admin_pool: Pool) -> None:
    ...
    try:
        async with _simulate_request_connection():
            uploaded = await upload_document(
                file=_md_upload_file("leave.md", _VALID_MD), section_role="hr", tenant_id=None
            )
        async with _simulate_request_connection():
            result = await reindex_documents(tenant_id=None)
    finally:
        middleware._request_session.reset(token)  # type: ignore[arg-type]

    assert result.chunk_count == uploaded.chunk_count == 1

    async with admin_pool.connection() as conn:
        await conn.execute("SELECT set_config('app.tenant_id', %s, true)", (str(tenant_id),))
        cur = await conn.execute("SELECT chunk_id FROM kb.chunks WHERE tenant_id = %s", (str(tenant_id),))
        rows = await cur.fetchall()
    assert [r[0] for r in rows] == [f"{uploaded.doc_id}#c1"], "re_index phải giữ nguyên chunk_id"


async def test_reindex_documents_empty_tenant_returns_zero(admin_pool: Pool) -> None:
